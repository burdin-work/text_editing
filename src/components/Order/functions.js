...
export const priceCalculation = (language, extension, textLength) => {
    ...
    const min_price = (language === 'en') ? 120 : 50;
...
export const countTime = (language,
    ...
    if (date && date.includes(', ')) {
        date = moment(date, 'DD/MM/YYYY, HH:mm ddd')
            .format('MMMM DD, YYYY HH:mm:ss');
    }

    // For testing a function with a different date
    const startDate = date ? moment(new Date(date)) : moment(new Date())
        .utc()
        .tz('Europe/Kiev');
    ...
    const workHoursToday = (hoursNow >= 19
        || moment(startDate)
            .isoWeekday() === 6
        || moment(startDate)
            .isoWeekday() === 7)
    ...
    const speed = (language === 'en') ? 333 : 1333;
    ...
    if (extension === 'other' && numHoursAllTime !== 0.99) numHoursAllTime += (numHoursAllTime / 100) * 20;
    ...
    const positionMinutes = String(hoursAfter10 + 0.00001)
        .split('.')[1][0];
    const numHours = (+positionMinutes >= 9) ? Math.round(hoursAfter10) : Math.floor(hoursAfter10);
    const closerTo0 = '019';
    const closerTo15 = '23';
    ...
    const desiredDate = moment(startDate)
        .isoAddWeekdaysFromSet({
            workdays: numDays,
            weekdays: [1, 2, 3, 4, 5],
            exclusions: [],
        });
